...
try {
  const res = await fetch(`${host}/createServer?server-name=${serverName}&server-type=${serverType}&server-version=${serverVersion}&min-ram=${minRam}&min-cpu=${minCpu}&max-ram=${maxRam}&max-cpu=${maxCpu}&storage=${storage}`, {
    method: 'POST',
    credentials: 'include'
  }) 
